...
export const DataRowsMelee = ({ row, columns }) => {
  ...
  if (
    !row["video_link"].includes("videos.gem360.in") &&
  ...
  const handleAddToCart = () => {
    if (diamonds_in_cart.some((item) => item.id === row.id)) {
      ...
      dispatch(
        addDiamondToCart({ ...row, amount_in_cart: parseInt(currentAmount) })
      );
    }
  };
  ...
  useState(() => {
    if (diamonds_in_cart.some((item) => item.id === row.id)) {
      setCurrentAmount(
        diamonds_in_cart.find((item) => item.id === row.id).amount_in_cart
      );
    } else {
      setCurrentAmount("");
    }
      ...
      <tr>
        {columns.map((column, colIndex) => (
          <>
            <td
              key={colIndex}
              className={`px-2 py-4 whitespace-nowrap text-text border-solid border-[1.5px] bg-black border-dark-grey ${
                column.field === "image" ? "align-middle text-center" : ""
              }`}
            >
              {column.field === "total" ? (
                <div className="w-[100%]">
                  {spotPrice ? (
                    <div className="flex flex-col">
                      <div className="flex">
                        <p className="">$ {calcTotalPrice(false)}</p>
                      </div>
                      <div className="flex">
                ...
                <div className="flex flex-col">
                  ...
                  <div className="flex">
                    <p className="">
                      R{" "}
                      {(
                        spotPrice / parseFloat(row.specifications.carat)
                      ).toFixed(2)}
                    </p>
                  </div>
                </div>
                ...
                <div className="w-full flex justify-center">
                  ...
                  >
                    {inCart ? (
                      ...
                      <ShoppingCartIcon
                        className={`w-5 h-5 ${
                          checkoutUnallowed ? "text-gray-200" : "text-white"
                        }`}
                      />
                    )}
                  </button>
                ...
                column.field === "information" ? (
                  column.renderCell({
                    value: { id: row.id, cert_id: row.cert_id },
                    row: row,
                  })
                ) : column.field === "delivery" ? (
                  column.renderCell({
                    location: warehouse.country,
                    from: warehouse.delivery_from,
                    to: warehouse.delivery_to,
                  })
                ) : (
