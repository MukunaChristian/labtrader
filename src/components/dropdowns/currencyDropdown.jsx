...
export const CurrencyDropdown = () => {
  ...
  return (
    <div ref={ref} className="relative w-28 h-8 rounded-md mr-4">
      ...
      <a className="show-rand" onClick={() => setIsOpen(!isOpen)}>
        
        ZAR

      </a>
      <ChevronDownIcon className="w-6 h-6 ml 1 text-white" />
      </div>
      <button
        ...
        <div className="absolute right-0 mt-2 w-72 bg-white border-solid border-[1.5px] rounded-sm">
          <div className="py-1">
            {options.map((option) => (
              ...
              >
                <img
                  className="w-12 h-8 mr-2 object-cover rounded-lg"
                  src={option.imgSrc}
                  alt={option.name}
                />
                <div className="flex flex-col">
                  ...
                  <p className="text-xs">
                    1 USD = {rates[option.code].toFixed(2)} {option.code}
                  </p>
                </div>
              </a>
            ))}
